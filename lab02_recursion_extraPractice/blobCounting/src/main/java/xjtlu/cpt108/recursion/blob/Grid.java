...
public class Grid {
    ...
    public int countCellsInBlob(int x, int y) {
        ...
        int[][] intGrid = to2DArray();
        return recursiveHelper(x, y, intGrid);

    }

    private int recursiveHelper(int x, int y, int[][] intGrid) {
        if (!isWithinBoundary(x, y) || intGrid[x][y] != 1) {
            return 0;
        } else {
            intGrid[x][y] = -1;
            return 1 + recursiveHelper(x - 1, y, intGrid)
                    + recursiveHelper(x - 1, y + 1, intGrid)
                    + recursiveHelper(x, y + 1, intGrid)
                    + recursiveHelper(x + 1, y + 1, intGrid)
                    + recursiveHelper(x + 1, y, intGrid)
                    + recursiveHelper(x + 1, y - 1, intGrid)
                    + recursiveHelper(x, y - 1, intGrid)
                    + recursiveHelper(x - 1, y - 1, intGrid);
        }
    ...
    private int[][] to2DArray() {
        ...
        for (int i = 0; i < rows.length; i++) {
            ...
            // Parse each row
            for (int j = 0; j < rows[i].length(); j++) {
                if (rows[i].charAt(j) == FILLED_CELL) {
                    map[i][j] = 1;
                } else {
                    map[i][j] = 0;
                }
